	...
	function GetPartners() {
		...
		$.getJSON('./includes/getMatches', {token:$('#super_user_key').val()}, function(data) {
			if(data.length == 0) {
				$("<h3>Matches you like will Appear here.</h3>").prependTo(".match-main");
			}else{
			$(".match-main").html('');
				$.each(data, function (i, item)  {
					let temp =	$('<a class="a-match-users" href="profile?key='+item.user_key +'" ><div class="match-users"> <div class="matchimg"><img src="'+item.profile_pic +'" height="90%"></div> <div  class="matchinfo"><Span>'+item.name +'</Span> <Span>'+item.country +'</Span></div></div></a>');
				  temp.prependTo(".match-main");
				});
			}
		});
	}
...
document.addEventListener("DOMContentLoaded", function(event) {
				 ...
				 function get_current_card_id(){
					 let carduserid = $('.stackedcards-active #userid');
					 let yes = carduserid.data("identifier");
    ...
    //On the actions onSwipeLeft, onSwipeRight and onSwipeTop you need to remove the currentPosition variable (currentPosition = currentPosition + 1) and the function setActiveHidden
      ...
      if(!(currentPosition >= maxElements)){
		...
		function gestureMove(evt) {
			...
			if(!(currentPosition >= maxElements)){
				evt.preventDefault();
				transformUi(translateX, translateY,0.5, currentElementObj);
	...
	function GetMatch(callback) {
		let usea =  $('#content');
		let user_main = usea.data("ke"); 
		$.getJSON('./includes/match', {token:user_main}, function(data) {

				$.each(data, function (i, item)  {
					let card =$('<div class="card-item " data-image="'+item.profile_pic +'">	<div class="meta-data"><div class="more">'+item.online +'</div><h2>'+item.name +', <b>'+item.userAge +'</b></h2><p>'+item.occupation +'</p></div><span id="userid" data-identifier="'+item.user_key +'"></span></div>');
					...
					card.prependTo(".stackedcards-container");	
